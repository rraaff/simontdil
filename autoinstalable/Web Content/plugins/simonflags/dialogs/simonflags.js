...
CKEDITOR.dialog.add( 'simonflags', function( editor )
{
	var config = editor.config,
		...
		images = config.simonflags_images,
		columns = config.simonflags_columns || 8,
		i;
	...
	var labelId = CKEDITOR.tools.getNextId() + '_simonflags_emtions_label';
	var html =
	[
		'<div>' +
		...
		html.push(
			'<td class="cke_dark_background cke_centered" style="vertical-align: middle;">' +
				'<a href="javascript:void(0)" role="option"',
					...
					'<img class="cke_hand" title="', config.simonflags_descriptions[i], '"' +
						' cke_src="', CKEDITOR.tools.htmlEncode( config.simonflags_path + images[ i ] ), '" alt="', config.simonflags_descriptions[i], '"',
						' src="', CKEDITOR.tools.htmlEncode( config.simonflags_path + images[ i ] ), '"',
						// IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
						( CKEDITOR.env.ie ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ),
					'>' +
					'<span id="' + smileyLabelId + '" class="cke_voice_label">' +config.simonflags_descriptions[ i ]  + '</span>' +
				'</a>',
 			'</td>' );
